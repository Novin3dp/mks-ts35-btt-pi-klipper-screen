...
REPO_URL="https://github.com/Novin3dp/mks-ts35-btt-pi-klipper-screen.git"
...
if ! grep -qE 'sun50i-h616|CB1|bigtreetech' /proc/device-tree/compatible 2>/dev/null; then
    echo "WARNING: target does not look like the tested CB1/H616 platform."
    echo "The installer will continue only if you explicitly confirm."
...
sudo mkdir -p "$INSTALL_DIR/scripts" "$INSTALL_DIR/overlay" "$INSTALL_DIR/services"
...
echo "$BACKUP_DIR" | sudo tee "$INSTALL_DIR/last_backup" >/dev/null

log "Compiling Device Tree Overlay"
dtc -@ -I dts -O dtb \
    -o "$INSTALL_DIR/overlay/ts35_cb1.dtbo" \
    "$INSTALL_DIR/overlay/ts35_cb1.dts"
...
if [[ -f /boot/armbianEnv.txt ]]; then
    ...
    fail "/boot/armbianEnv.txt not found. This installer targets Armbian/CB1."
fi
...
chown -R "$USER_NAME:$USER_NAME" "$USER_HOME/KlipperScreen"
...
BEEPER_ESCAPED="$FLAG_FILE"
sed -e "s#__TS35_BEEPER_FLAG__#$BEEPER_ESCAPED#g" \
    "$PROJECT_DIR/services/virtual-touch.service" | sudo tee /etc/systemd/system/virtual-touch.service >/dev/null

log "Installing beeper watcher"
sed -e "s#__TS35_USER__#$USER_NAME#g" -e "s#__TS35_BEEPER_FLAG__#$BEEPER_ESCAPED#g" \
...
log "Installing Klipper Touch Beep macro when printer.cfg exists"
PRINTER_CFG="$USER_HOME/printer_data/config/printer.cfg"
if [[ -f "$PRINTER_CFG" ]] && ! grep -q '^\[gcode_macro TOGGLE_BEEPER\]' "$PRINTER_CFG"; then
    cat "$PROJECT_DIR/klipper/toggle_beeper.cfg" | sudo tee -a "$PRINTER_CFG" >/dev/null
...
if [[ -f "$KS_CFG" ]] && ! grep -q '^\[menu __main more beeper_toggle\]' "$KS_CFG"; then
    ...
    if grep -qF "$MARKER" "$KS_CFG"; then
        sudo sed -i "/$MARKER/r $PROJECT_DIR/klipperscreen/touch_beep.conf" "$KS_CFG"
        sudo sed -i "/$MARKER/ { /$MARKER/!b; }" "$KS_CFG" || true
    else
        cat "$PROJECT_DIR/klipperscreen/touch_beep.conf" | sudo tee -a "$KS_CFG" >/dev/null
    fi
